...
3. `Failsafe Cluster`：出现异常直接忽略，吸入审计日志
...
3. 在服务提供者实现类中方法上添加`@HystrixCommand`注解，让Hystrix代理实现类
```java
...
4. 在服务消费者调用服务提供者的方法上添加`@HystrixCommand`注解并指定`fallbackMethod`属性，重写`fallbackMethod`指定的方法
```java
